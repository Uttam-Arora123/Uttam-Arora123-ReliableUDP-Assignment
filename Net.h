...
#include <unistd.h>
	...
	public:
		...
		bool operator < (const Address& other) const
		{
			// note: changes are done so we can use address as a key
	...
	// we have used packet queue to store the information about the packet being send and recieved in a sorted manner
	...
	// above unsigned int sequence determine packet sequence number
	// above int size determine packet size in bytes
...
#ifdef NET_UNIT_TEST
		unsigned int packet_loss_mask;
#endif

		ReliabilitySystem reliabilitySystem;
	};// the above reliability system: manages sequence numbers and acks, tracks network stats etc
}
